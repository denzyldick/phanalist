        ...
        for (key, violations) in &results.files {
            for violation in violations {
                ...
                let region = sarif::Region {
                    ...
                    char_offset: None,
                    end_column: Some(
                        (violation.span.column as i64 + violation.span.position as i64).max(1),
                    ),
                    end_line: Some(
                        (violation.span.line as i64 + violation.span.position as i64).max(1),
                    ),
                    message: None,
                    properties: None,
                    snippet: None,
                    source_language: Some("PHP".to_string()),
                    start_column: Some((violation.span.column as i64).max(1)),
                    start_line: Some((violation.span.line as i64).max(1)),
                };
